...
<p>This website is dedicated to sharing both theoretical and practical work I produce in software DSP (that is, digital signal processing, e.g. audio and image, done in software). There's also a blog that I use for exploring some contemporary topics that I find interesting, particularly in the context of audio DSP, e.g. procedural sound generation, or some projects that I'm involved in.</p>

<p>My long term passions are related to procedural sound generation as its a topic that hasn't been explored too much and which can be mathematically (as well as aesthetically) rather involved. The benefits are however fairly reasonable since sample-based sound can be limited in the sense of interactivity and missing realism that's achievable only using computational, rather than artistic methods. As it has been demonstrated e.g. by these two projects:</p>

<p>I'm also interested in novel and esoteric, unbeforeheard/-seen signal processing algorithms such as those blending the boundary of image and audio signal processing algorithms, as demonstrated in the popular program Metasynth, or synthetic methods that display a high level of realism.</p>

<p> I have no fixed set of tools that I work with, but I have found that the ones that are either commonly used for these kinds of programs are C++ (for plug-in work I like the WDL-OL framework) and at the higher level one may use Python (Scipy). For audio signal processing I've found CSound to be a particularly interesting API, since it runs on PC, is written in C++ and has a large existing library of sound processing code with some fairly esoteric algorithms as well. Pure Data is OK, but at the time of writing it has some weird multiple instantiation related problems, so I'm not sure about messing with Pure Data. Therefore my development efforts are mainly focused on these mentioned tools that I find fruitful for further development. I also have an interest in audio API interaction with physics engines, which is something that I'll be looking as well. The code you will find in this site will be mainly using these. I prefer to work and support non-commercial and open source efforts.</p>

<p> From the Commercial and Open Source sections you can find signal processing software that I have developed. The commercial software is either propietarily licensed (source not available) or available for a fee (e.g. a software library). The open source software is hosted in Github and is free access with licenses varying on a project basis.
</p>
